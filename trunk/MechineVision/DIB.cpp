...
/*ofstream fout("test10.txt");
fout<<cf.GetLength()<<endl;
fout<<dwDibSize<<endl;*/
...
/*	ofstream fout("±m¦â.txt");
	int o=0;
	while(o != 10000)
	{
       fout<<(int)m_pDib[o]<<endl;
	   o++;
	}
	fout.close();
*/
...
BOOL CDib::Save2(unsigned char * array, const char *pszFilename )
{
/*	for(int i=0;i<m_pBIH->biHeight ;i++)
		for(int j=0;j<m_pBIH->biWidth ;j++)
			array[i*2160+j]=m_pDib[i*2160+j+1064];*/

	for(int i=0;i<m_pBIH->biHeight ;i++)
		for(int j=0;j<m_pBIH->biWidth ;j++)
			m_pDib[i*m_pBIH->biWidth+j+1064]=array[i*m_pBIH->biWidth+j];
...
BOOL CDib::Draw2( CDC *pDC, int nX, int nY, int nWidth, int nHeight ,unsigned char *array, int Width, int Height)
{  
	if( m_pDib == NULL )
		return( FALSE );
	for(int i=0;i<Height ;i++)
		...
		{
			m_pDib[i * Width+j+1064]=array[i * Width+j];
		}
	...
	// to the value that's in the BITMAPINFOHEADER.
	if( nWidth == -1 )
		nWidth =Width;//nWidth = m_pBIH->biWidth;
	if( nHeight == -1 )
		nHeight =Height;//nHeight = m_pBIH->biHeight;
...
int ** CDib::Get_Image_Array(int height,int width)
{
	int **array;
	...
	for(int i=0;i<height;i++)
		...
		for(int i=0;i<height;i++)
			for(int j=0;j<width;j++)
			{  
				array[i][j]=(int)m_pDib[i*width+j+1064];
			}
			return array;
}

void CDib::CCopyImage(unsigned char *array4)
{
  unsigned char *temp1;
  temp1=(m_pDib+1064);
  memcpy(array4,temp1,(m_pBIH->biWidth * m_pBIH->biHeight));
